...
echo $yournameorBU;
docker run -dti --name=azure-cli-python-$yournameorBU --restart=always azuresdk/azure-cli-python && docker cp terraform azure-cli-python-$yournameorBU:/root && docker cp id_rsa.pub azure-cli-python-$yournameorBU:/root && docker exec -ti azure-cli-python-$yournameorBU bash -c "az login && cd ~/ && git clone https://github.com/dwaiba/aks-terraform && cd ~/aks-terraform && git checkout aks-terraform-rbac && cp ~/id_rsa.pub ~/aks-terraform && cp ~/terraform /usr/bin && terraform init && terraform plan -out run.plan && terraform apply "run.plan" && bash";
